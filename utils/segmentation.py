...
def run(
    ...
    mean, std = echonet.utils.get_mean_and_std(echonet.datasets.Echo(root=data_dir, split="train", target_type=[]))
    ...
    with torch.no_grad():
        ...
        lower = max(0, min(y.min(), yhat.min()))
        upper = min(100, max(y.max(), yhat.max()))
        ...
        # TODO: comparison (move to function
        tasks = ["LargeFrame", "SmallFrame", "LargeTrace", "SmallTrace"]
        kwargs = {"target_type": tasks,
                  "mean": mean,
                  "std": std
                  }
        dataset = echonet.datasets.Echo(root=data_dir, split="test", **kwargs)
        dataloader = torch.utils.data.DataLoader(dataset,
                                                 batch_size=batch_size, num_workers=num_workers, shuffle=False, pin_memory=(device.type == "cuda"))

        index = 0
        os.makedirs(os.path.join(output, "comparison", "fixed"), exist_ok=True)
        os.makedirs(os.path.join(output, "comparison", "random"), exist_ok=True)
        with open(os.path.join(output, "comparison", "random", "seed.csv"), "w") as log:
            log.write("index,computer\n")
            with torch.set_grad_enabled(False):
                with tqdm.tqdm(total=len(dataloader), desc="Comparison") as pbar:
                    for (_, (large_frame, small_frame, large_trace, small_trace)) in dataloader:
                        ...
                        for (lf, sf, lt, st, lp, sp) in zip(large_frame.cpu().numpy(), small_frame.cpu().numpy(), large_trace.cpu().numpy(), small_trace.cpu().numpy(), y_large.cpu().numpy(), y_small.cpu().numpy()):
                            swap = random.random() < 0.5
                            log.write("{},{}\n".format(index, swap))
                            row_fixed = []
                            row_rand = []
                            for (frame, t, p) in [(lf, lt, lp), (sf, st, sp)]:
                                frame = (frame * std.reshape((3, 1, 1)) + mean.reshape((3, 1, 1))).clip(0, 255).astype(np.uint8)
                                ...
                                left[2, :, :] = np.maximum(left[2, :, :], (255 * t).astype(np.uint8))
                                right[0, :, :] = np.maximum(right[0, :, :], (255 * p).astype(np.uint8))
                                row_fixed.append(np.concatenate((left, right), axis=2))
                                ...
                                left[2, :, :] = np.maximum(left[2, :, :], (255 * t).astype(np.uint8))
                                right[0, :, :] = np.maximum(right[0, :, :], (255 * p).astype(np.uint8))
                                row_rand.append(np.concatenate((left, right), axis=2))

                            img = np.concatenate(row_fixed, axis=1)
                            img = img.transpose((1, 2, 0))
                            PIL.Image.fromarray(img).save(os.path.join(output, "comparison", "fixed", "{:06d}.jpg".format(index)))

                            img = np.concatenate(row_rand, axis=1)
                            img = img.transpose((1, 2, 0))
                            PIL.Image.fromarray(img).save(os.path.join(output, "comparison", "random", "{:06d}.jpg".format(index)))
                            index += 1
